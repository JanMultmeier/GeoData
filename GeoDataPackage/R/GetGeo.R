...
getGeo <- function(addr,source="google",local=FALSE,country="Germany") {
  if ((source %in% c("google","dstk","arcgis"))==FALSE) {stop("Unbekanntes 'source'-Argument: google, dstk oder arcgis sind erlaubt")}
  require(rjson)
  results <- data.frame("lat"=numeric(length=length(addr)),"lon"=numeric(length=length(addr)))
    if (source=="google") {
      for (i in seq_along(addr)) {
        ...
        if (attempts==3) {stop("Abfragelimit erreicht")}
      }
    }
  }
  if (source=="dstk") {
    ifelse(local==TRUE,api_base <- "http://localhost:8080",api_base <- "http://www.datasciencetoolkit.org")
    if (length(country!=length(addr))) {country <- rep(country, len=length(addr))}
    for (i in seq_along(addr)) {
      success <- FALSE
      attempts <- 0
      while (success==FALSE & attempts<100) {
        attempts <- attempts + 1
        url <- paste0(api_base,"/maps/api/geocode/json?sensor=false&address=",gsub(" ","+",addr[i]),",+",country[i])
        data <- fromJSON(file=url)
        if (data$status=="OK") {
          results$lat[i] <- data$results[[1]]$geometry$location$lat
          results$lon[i] <- data$results[[1]]$geometry$location$lng
  ...
  if (source=="arcgis") {
    for (i in seq_along(addr)) {
      url <- paste0("http://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/find?text=",gsub(" ","+",addr[i]),"&f=json")
      data <- fromJSON(file=url)
      results$lat[i] <- data$locations[[1]]$feature$geometry$y
